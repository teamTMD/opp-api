...
def test_put_update_payment_type():
    ...
    payment_type_id = 4
    response = client.put(
        f"/payment_type/{payment_type_id}", headers=headers, json=updated_payment_type
    )
...
def test_validate_card_valid():
    ...
    response = client.post(
        "/payment_type/validate-card",
        params={"card_number": VALID_CARD},
        headers=headers,
    )
...
def test_process_card_not_fradulent():
    ...
    response = client.post(
        "/payment_type/process-card",
        params={"card_number": VALID_CARD, "amt": AMOUNT},
        headers=headers,
    )
